...
import Impl.GetUserIDByTokenMessagePlanner
import Impl.DeleteUserMessagePlanner
  ...
  private def executePlan(messageType: String, str: String): IO[String] =
    messageType match {
      case "GetUserIDByTokenMessage" =>
        IO(
          decode[GetUserIDByTokenMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for GetUserIDByTokenMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
       
      case "DeleteUserMessage" =>
        IO(
          decode[DeleteUserMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for DeleteUserMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
