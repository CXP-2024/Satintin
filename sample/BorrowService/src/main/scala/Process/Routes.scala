...
import Impl.RenewBookMessagePlanner
import Impl.ReturnBookMessagePlanner
import Impl.BorrowBookMessagePlanner
import Impl.QueryBorrowRecordsMessagePlanner
...
object Routes:
  ...
  private def executePlan(messageType: String, str: String): IO[String] =
    messageType match {
      case "RenewBookMessage" =>
        IO(
          decode[RenewBookMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for RenewBookMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
       
      case "ReturnBookMessage" =>
        IO(
          decode[ReturnBookMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for ReturnBookMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
       
      case "BorrowBookMessage" =>
        IO(
          decode[BorrowBookMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for BorrowBookMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
       
      case "QueryBorrowRecordsMessage" =>
        IO(
          decode[QueryBorrowRecordsMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for QueryBorrowRecordsMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
