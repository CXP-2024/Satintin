...
import Impl.CheckUserQuotaMessagePlanner
import Impl.QueryBorrowQuotaMessagePlanner
...
object Routes:
  ...
  private def executePlan(messageType: String, str: String): IO[String] =
    messageType match {
      case "CheckUserQuotaMessage" =>
        IO(
          decode[CheckUserQuotaMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for CheckUserQuotaMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
       
      case "QueryBorrowQuotaMessage" =>
        IO(
          decode[QueryBorrowQuotaMessagePlanner](str) match
            case Left(err) => err.printStackTrace(); throw new Exception(s"Invalid JSON for QueryBorrowQuotaMessage[${err.getMessage}]")
            case Right(value) => value.fullPlan.map(_.asJson.toString)
        ).flatten
